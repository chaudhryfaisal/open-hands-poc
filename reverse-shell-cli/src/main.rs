...
#[command(about = "A secure reverse shell CLI with WebSocket and SSL support")]
struct Args {
    #[arg(long, default_value = "localhost:8081")]
    server: String,

    #[arg(long, default_value = "admin_token_456")]
    token: String,
...
async fn main() -> Result<()> {
    ...
    let cli = ReverseShellCli::new(args.server, args.token, args.ssl);
    cli.start().await?;
